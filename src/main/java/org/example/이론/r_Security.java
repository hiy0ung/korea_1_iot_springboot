...
public class r_Security {
    /*
        1. WebSecurityConfig
        : Spring Security의 설정을 담당하는 파일 (전반적인 보안 정책을 정의)
        - 인증 방식(JWT 사용), 인증이 필요한 URL, 필터 순서를 설정

        - 흐름 -
        : 애플리케이션 실행 시 Spring Boot는 해당 설정 파일을 기반으로 보안 관련 설정을 적용
        : JwtAuthenticationFilter 와 같은 필터들을 언제, 어떻게 적용할 지 설정
        ...
        : 이후 사용자가 보낸 요청에 JWT 토큰을 추출하면 JwtProvider가 토큰의 유효성을 검증
        ...
        : 사용자가 이후 요청 시 Authorization 헤더에 "Bearer 토큰 형식"으로 JWT 토큰을 전달
        : JwtAuthenticationFilter는 요청을 가로채 Authorization 헤더에서 토큰을 추출

        cf) 로그인 시 - JwtProvider (토큰 X)
            인증, 인가 처리가 필요 시 - JwtAuthenticationFilter (토큰 O)
