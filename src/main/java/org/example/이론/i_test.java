...
public class i_test {
    /*
        테스트 코드
        1. 테스트 코드란?
        : 애플리케이션이 의도한대로 작동하는지 확인하기 위해 작성된 "코드"
        ...
        cf) 테스트 코드는 src 디렉터리 내의 test 디렉터리에서 작업
        ...
        cf) Assertion.asserEquals(sum, a + b);
        기댓값: sum
        실제 비교값: a + b
        >> 코드만 보고 기댓값과 비교값이 잘 구분되지 X

        AssertJ 를 사용하여 가독성 향상
        cf) assertThat(a + b).isEqualTo(sum);
        : a와 b를 더한 값이 sum과 같아야 한다.
