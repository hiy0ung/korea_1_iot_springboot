...
public class h_dispatchServlet {
    /*
        ...
        >> HttpServletRequest / HttpServletResponse (요청과 응답(반환))
        : 웹 애플리케이션의 요청 흐름을 관리하는 중심 역할
        ...
        2. Handle(Controller) 조회
        : HandlerMapping을 통해 요청 URI에 맞는 "핸들러"를 찾음
        EX) /student로 요청 -> StudentController가 매핑

        3. Handle Adapter 조회
        : 매핑한 핸들러 실행을 위한 어댑터를 찾고 Controller가 실행될 수 있는 형태로 어탭터가 변환
